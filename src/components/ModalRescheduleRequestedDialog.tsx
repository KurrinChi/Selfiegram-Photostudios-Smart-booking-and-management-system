...
interface ModalRescheduleRequestedDialogProps {
   isOpen: boolean;
  onClose: () => void;
  data: {
    id: string;
...
const ModalRescheduleRequestedDialog: React.FC<ModalRescheduleRequestedDialogProps> = ({
  isOpen,
  onClose,
  data,
}) => {
    ...
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex justify-center items-center p-4">
      <div className="bg-white text-gray-900 rounded-lg shadow-md max-w-lg w-full p-6">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h1 className="text-lg font-bold">Reschedule Requested</h1>
        </div>
       ...
       {/* Request Date */}
        ...
        {/* Booking ID */}
            ...
            {/* Booking Date */}
            <div className="mb-2 text-sm text-gray-500">
            <span className="font-semibold">New Booking Date: </span>
            {formatDate(data.bookingDate)}
            </div>

            {/* Booking Time */}
            <div className="mb-4 text-sm text-gray-500">
            <span className="font-semibold">New Booking Time: </span>
            {data.bookingTime}
            </div>

        {/* Message */}
        <p className="mb-4 text-sm text-gray-700">
          Your appointment reschedule request has been submitted successfully. We will notify you once it has been processed.
        </p>
...
export default ModalRescheduleRequestedDialog;
