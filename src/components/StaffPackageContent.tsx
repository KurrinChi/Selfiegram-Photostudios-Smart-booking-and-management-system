...
const ClientPackagePageContent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetchWithAuth(`${API_URL}/api/staff/packages`);
        const data = await response.json();
        const arr = Array.isArray(data) ? data : [];
        const normalized: Package[] = arr
          .map((p: any) => {
            // Coerce numeric fields
            const price = Number(p.price);
            const baseRaw =
              p.base_price ?? p.basePrice ?? p.original_price ?? p.price; // fallbacks
            ...
            let is_discounted =
              p.is_discounted !== undefined
                ? Number(p.is_discounted)
                : undefined;
        ...
        if (normalized.length && normalized[0]) {
          ...
          console.debug(
            "[ClientPackages] Sample normalized package",
            normalized[0]
          );
        }
