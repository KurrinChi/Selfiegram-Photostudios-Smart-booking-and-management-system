    ...
    fetch(`${API_URL}/api/admin/users`, {
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          console.log('[Broadcast] Raw users fetched:', data.length, data.slice(0,10));
          const customers = data
            .filter((u: any) => {
              const t = (u.userType || u.usertype || '').toString().trim().toLowerCase();
              return t === 'customer';
            })
            .map((u: any) => ({
              userID: u.userID,
          ...
          console.log('[Broadcast] Filtered customers (customerList) length:', customers.length, customers);
